...
const ticketCategories = [
  ...
  { key: 'premium', label: 'Premium', multiplier: 2 }
];
...
const EventDetails = () => {
  ...
  const unitPrice = useMemo(
    () => (event ? Number(event.price) * Number(category.multiplier) : 0),
    [event, category]
  );
  const total = useMemo(() => unitPrice * Number(quantity || 0), [unitPrice, quantity]);
  ...
  return (
    <main className="mx-auto grid max-w-6xl gap-8 px-6 py-12 lg:grid-cols-2">
      ...
      <section className="space-y-4">
            ...
            {ticketCategories.map((item) => (
              <button
                ...
                className={`rounded-lg border px-3 py-2 text-sm ${
                  category.key === item.key ? 'border-cyan-300 bg-cyan-400/20' : 'border-white/20 bg-slate-900'
                }`}
              >
                {item.label} (${(Number(event.price) * item.multiplier).toFixed(2)})
              </button>
            ))}
          </div>
        </div>
        ...
        <Link
          to="/checkout"
          state={{ event, quantity, total, ticketCategory: category.label, unitPrice }}
