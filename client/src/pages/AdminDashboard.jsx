...
import api, { adminHeaders } from '../services/api';
...
const AdminDashboard = () => {
  ...
  const loadData = async () => {
    try {
      const [eventsRes, bookingsRes] = await Promise.all([api.get('/events'), api.get('/bookings', adminHeaders())]);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await api.put(`/events/${editId}`, payload, adminHeaders());
      } else {
        await api.post('/events', payload, adminHeaders());
      }
      setForm(initialForm);
      setEditId(null);
      loadData();
  ...
  const onDelete = async (id) => {
    try {
      await api.delete(`/events/${id}`, adminHeaders());
      loadData();
    } catch (err) {
  ...
  return (
    <main className="mx-auto grid max-w-6xl gap-8 px-6 py-12 lg:grid-cols-2">
      <section>
        ...
        {missingAdminKey ? <p className="mb-3 text-sm text-amber-300">Set VITE_ADMIN_API_KEY to use admin APIs.</p> : null}
