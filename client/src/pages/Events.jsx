...
const Events = () => {
  ...
  const { seatUpdates, lockUpdates } = useSocketData();
  ...
  const mergedEvents = useMemo(
    () =>
      events.map((event) => {
        const dbAvailability = seatUpdates[event.id] ?? event.available_seats;
        const lockAvailability = lockUpdates[event.id]?.effectiveAvailableSeats;
        return {
          ...event,
          available_seats: lockAvailability ?? dbAvailability
        };
      }),
    [events, seatUpdates, lockUpdates]
  );
